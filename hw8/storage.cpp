...
void Storage::addReview(Review* r){
	...
	std::vector<Product*> reviewedProd;
	if(reviewedProducts.find(findUser(r -> user)) == reviewedProducts.end()){
		for(unsigned int i = 0; i < products_.size(); i++){
			if(products_[i] -> getName() == name){
				reviewedProd.push_back(products_[i]);
			}
		} 
	}
	else{
		reviewedProd = reviewedProducts[findUser(r -> user)];
		for(unsigned int i = 0; i < products_.size(); i++){
			if(products_[i] -> getName() == name){
				reviewedProd.push_back(products_[i]);
			}
	...
	reviewedProducts[findUser(r -> user)] = reviewedProd;
...
std::map<User*, std::vector<Product*> > Storage::getReviewedProducts(){
